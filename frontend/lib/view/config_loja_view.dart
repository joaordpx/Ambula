...
class _ConfigLojaViewState extends State<ConfigLojaView> {
  ...
  void initState() {
    ...
    // id da loja vindo da HomeVendedorView; se não vier, fallback 1
    _lojaId = widget.lojaId ?? 1;

    _futureDetalhe = _lojaService.getDetalhesLoja(_lojaId);

    // carrega categorias uma vez (GET /categorias-produto)
    _carregarCategorias();
  }
  ...
  Future<void> _salvar() async {
    ...
    try {
      // integra com PUT /api/lojas/{id}
      await _lojaService.atualizarLoja(
        lojaId: _lojaId,
        nome: nome,
        descricao: desc,
        status: _recebendoPedidos,
      );

      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Dados da loja salvos com sucesso.')),
      );
    } catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
